...
const RippleButton: React.FC<RippleButtonProps> = ({
  ...
  const createJsRipple = (event: MouseEvent<HTMLButtonElement>) => {
    ...
    setTimeout(() => {
      setJsRipples((currentRipples) => currentRipples.filter((ripple) => ripple.key !== newRipple.key));
    }, rippleDuration);
  };
      ...
      <>
        ...
        <button className={hoverButtonFinalClassName} onClick={handleButtonClick} disabled={disabled}>
          ...
          >
            ...
            <div className="hover-variant-visual-ripple pointer-events-none absolute w-0 h-0 rounded-full -translate-x-1/2 -translate-y-1/2 top-0 left-0 z-[-1]" />
          </div>
        </button>
      </>
    );
      ...
      <>
        ...
        <button className={hoverBorderButtonFinalClassName} onClick={handleButtonClick} disabled={disabled}>
          ...
          >
            ...
            <div className="hoverborder-variant-visual-ripple pointer-events-none absolute w-0 h-0 rounded-full -translate-x-1/2 -translate-y-1/2 top-0 left-0 z-[-1]" />
          </div>
        </button>
      </>
    );
